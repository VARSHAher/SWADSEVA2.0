    ...
    const fetchData = async () => {
      try {
        const { data } = await axios.get(
          "https://swadseva.onrender.com/api/menu",
        );
        setMenuItems(data);
        setLoading(false);
      } catch (err) {
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure?")) {
      try {
        ...
        await axios.delete(`https://swadseva.onrender.com/api/menu/${id}`, {
  ...
  const filteredItems = useMemo(() => {
    let items = menuItems.filter((item) => {
      ...
      const matchesCategory =
        selectedCategory === "All" || item.healthCategory === selectedCategory;
      const matchesSearch =
        query === "" ||
        item.name?.toLowerCase().includes(query) ||
        item.description?.toLowerCase().includes(query);
      const matchesFoodType =
        foodFilter === "all" || item.foodType === foodFilter;
      ...
      <div className="w-full min-h-screen bg-white shadow-sm p-6 md:p-12 relative">
        <div className="pt-20 mb-8 text-center flex flex-col items-center">
          ...
          <p className="text-slate-400 text-lg font-medium max-w-lg leading-relaxed">
            Discover dietitian-approved meals specifically calibrated for your
            clinical recovery.
          </p>
        </div>
              ...
              return (
                ...
                >
                  ...
                  <div className="absolute top-4 left-4">
                    <div
                      className={`w-5 h-5 border-2 ${isNonVeg ? "border-red-600" : "border-[#75a74c]"} flex items-center justify-center bg-white/10 backdrop-blur-md rounded-sm`}
                    >
                      <div
                        className={`w-2 h-2 ${isNonVeg ? "bg-red-600" : "bg-[#75a74c]"} rounded-full`}
                      ></div>
                    </div>
                  </div>
                  ...
                  <div className="absolute inset-0 p-6 flex flex-col justify-end transition-opacity duration-300">
                    <div className="mb-4">
                      ...
                      <p className="text-slate-300 text-xs font-medium line-clamp-2 mb-4 leading-relaxed">
                        {item.description}
                      </p>
                    </div>
                    ...
                    <div className="flex justify-between items-end w-full">
                      <div className="flex items-baseline gap-1">
                        <span className="text-[#75a74c] text-sm font-black">
                          ₹
                        </span>
                      ...
                      <div className="relative">
                        {isAdmin ? (
                          ...
                          <div className="flex items-center gap-1 bg-white px-3 py-2 rounded-xl shadow-2xl">
                            ...
                            <span className="text-[#1e4a6e] text-sm font-black">
                              {item.ratings || "4.0"}
                            </span>
                          </div>
                        )}
                      ...
                      <div className="transform -translate-y-2 group-hover:translate-y-0 transition-transform duration-500">
                        ...
                        <div className="flex gap-6 mb-10 text-white">
                          <div className="text-center">
                            <p className="text-[8px] uppercase text-white/60 mb-1 font-bold">
                              Prot
                            </p>
