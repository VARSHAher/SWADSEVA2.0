...
const AdminOrders = () => {
  ...
  const fetchOrders = async () => {
    try {
      const { data } = await axios.get(
        "https://swadseva.onrender.com/api/orders/all",
        {
  ...
  if (loading)
    return (
      <div className="flex justify-center items-center h-screen font-black text-slate-400 uppercase tracking-widest">
        Loading Dashboard...
      </div>
    );
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredOrders.length > 0 ? (
                filteredOrders.map((order) => {
                  const statusInfo = getAutoStatus(
                    order.createdAt,
                    order.status,
                  );
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        <span
                          className={`px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest ${statusInfo.color}`}
                        >
                          {statusInfo.label}
                        </span>
                      </td>
                      ...
                      <td className="px-6 py-4">
                        <p className="text-xs font-black text-[#1e4a6e]">
                          ₹{order.totalPrice}
                        </p>
                      </td>
