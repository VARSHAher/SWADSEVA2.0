...
const Orders = () => {
  ...
  const fetchOrders = useCallback(async () => {
    try {
      const { data } = await axios.get(
        "https://swadseva.onrender.com/api/orders",
        {
  ...
  const handleCancelOrder = async (order) => {
    ...
    if (mins > 15) {
      toast.error(
        "Order is already in the delivery phase and cannot be cancelled.",
      );
      return;
    }
    try {
      await axios.put(
        `https://swadseva.onrender.com/api/orders/${order._id}/status`,
        { status: "Cancelled" },
        { headers: { Authorization: `Bearer ${userInfo.token}` } },
      );
  ...
  if (loading)
    return (
      <div className="p-10 text-center font-black text-slate-400 text-[11px] uppercase tracking-widest bg-white min-h-screen pt-32">
        Loading SwadSeva...
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-white md:bg-[#f1f3f6] pb-20 font-['Outfit',_sans-serif] pt-24">
      ...
      <div className="max-w-3xl mx-auto px-4 mb-6">
        <h1 className="text-3xl font-[1000] text-[#1e4a6e] uppercase italic tracking-tighter">
          My <span className="text-[#75a74c]">Orders</span>
        </h1>
      </div>
      ...
      <div className="max-w-3xl mx-auto">
        {orders.length === 0 && (
          <div className="text-center py-20 px-8 bg-white rounded-[32px] border border-slate-100 shadow-xl shadow-slate-100/50 mx-4">
            <h2
              className="text-4xl md:text-5xl font-normal text-[#1e4a6e] tracking-tight leading-tight mb-4"
              style={{ fontFamily: "'Titan One', cursive" }}
            >
              Hungry for <br />{" "}
              <span className="text-[#75a74c]">Something?</span>
            </h2>
            <p className="text-sm font-bold text-slate-400 mt-3 mb-10 max-w-md mx-auto leading-relaxed">
              Your orders with SwadSeva will appear here. Go ahead and find some
              awesome food items!
            </p>
            <img
            ...
            <div className="mt-10 pt-8 border-t border-slate-100">
              <h3 className="text-xl font-black text-slate-300 tracking-widest uppercase">
                No Orders Yet
              </h3>
            </div>
          </div>
          ...
          {filteredOrders.map((order) => {
            const currentStatus =
              order.status === "Cancelled"
                ? "Cancelled"
                : getDynamicStatus(order.createdAt);
            ...
            if (currentStatus === "Placed") timeText = "25-30 Mins";
              ...
              timeText = "15 Mins Away";

            if (isDelivered || isCancelled) {
              return (
                <div
                  key={order._id}
                  className="bg-white p-5 rounded-2xl border border-slate-200 flex gap-4 transition-all hover:shadow-md mb-4"
                >
                  <img
                  ...
                  <div className="flex-1">
                    <p className="text-[9px] font-black text-[#75a74c] uppercase tracking-widest mb-1">
                      ID: #{orderIdShort}
                    </p>

                    <div className="flex items-center gap-2 text-[13px] font-black">
                      <div
                        className={`w-2 h-2 rounded-full ${isCancelled ? "bg-red-500" : "bg-[#75a74c]"}`}
                      ></div>
                      <span
                        className={
                          isCancelled ? "text-red-600" : "text-slate-900"
                        }
                      >
                        {isCancelled
                          ? "Cancelled"
                        ...
                        <div className="flex items-center gap-1.5">
                          {[1, 2, 3, 4, 5].map((s) => (
                            <FaStar
                              ...
                              onClick={() =>
                                setReviews({ ...reviews, [order._id]: s })
                              }
                            />
                          ))}
                        </div>
                        <span className="text-[11px] font-black text-[#75a74c] uppercase tracking-wider">
                          Rate Order
                        </span>
                      </div>
                    )}
            ...
            return (
              <div
                key={order._id}
                className="bg-white rounded-[28px] overflow-hidden shadow-xl shadow-slate-200/50 border border-slate-100 mb-6"
              >
                <div className="p-6">
                  <div className="flex justify-between items-start mb-6">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-[9px] font-black text-[#75a74c] uppercase tracking-widest">
                          ID: #{orderIdShort}
                        </p>
                        <h3 className="text-[18px] font-[1000] text-slate-900 tracking-tighter">
                          Arriving at:
                        </h3>
                        <p className="text-[11px] font-bold text-slate-400 truncate max-w-[180px]">
                    ...
                    <div className="text-right">
                      <span
                        className={`text-[10px] font-[1000] px-3 py-1.5 rounded-full uppercase tracking-widest animate-pulse ${isCancelled ? "bg-red-100 text-red-600" : "bg-blue-100 text-[#1e4a6e]"}`}
                      >
                        {currentStatus}
                      </span>
                      {!isCancelled && (
                  ...
                  <div className="space-y-3 bg-slate-50 p-4 rounded-2xl border border-slate-100 mb-6">
                    ...
                    <div className="pt-2 mt-2 border-t border-slate-200 flex justify-between items-center">
                      ...
                      <span className="text-[14px] font-[1000] text-[#1e4a6e]">
