...
export default function Contact({ isAdmin }) {
      ...
      const fetchInquiries = async () => {
        try {
          ...
          const res = await axios.get(
            "https://swadseva.onrender.com/api/inquiries",
            config,
          );
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      await axios.post("https://swadseva.onrender.com/api/inquiries", formData);
      toast.success(
        "Request submitted successfully. Our dietician will call you.",
      );
      target.reset();
    } catch (err) {
  ...
  const handleResolve = async (id) => {
    try {
      await axios.delete(`https://swadseva.onrender.com/api/inquiries/${id}`, {
        headers: { Authorization: `Bearer ${userInfo?.token}` },
      });
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4 bg-white p-6 rounded-3xl shadow-sm border border-slate-200">
            <div>
              <h1 className="text-xl font-black text-slate-800 uppercase tracking-tight flex items-center gap-2">
                <MessageSquare size={22} className="text-[#1e4a6e]" /> Support
                Inquiries
              </h1>
              ...
              <tbody className="divide-y divide-slate-50">
                {inquiries.length === 0 ? (
                  <tr>
                    <td
                      colSpan="4"
                      className="p-20 text-center text-slate-400 font-bold uppercase text-xs tracking-widest"
                    >
                  ...
                  inquiries.map((iq) => (
                    ...
                    >
                      <td className="p-8">
                        <p className="font-black text-[#1e4a6e] uppercase text-sm">
                          {iq.name}
                        </p>
                      ...
                      <td className="p-8 text-sm font-medium text-slate-500 max-w-xs truncate">
                        {iq.message}
                      </td>
      ...
      <div className="max-w-7xl mx-auto pb-24 px-6 grid lg:grid-cols-12 gap-12 items-start">
        <div className="lg:col-span-4 space-y-8">
          <div className="bg-slate-50/50 border border-slate-100 p-10 rounded-[2.5rem] space-y-10">
            <h3 className="text-[10px] font-black uppercase tracking-[0.3em] text-[#75a74c]">
              Contact Details
            </h3>
              ...
              {[
                {
                  icon: <Phone size={20} />,
                  label: "Call Us",
                  val: "+91 1800-SWAD-SEVA",
                },
                {
                  icon: <Mail size={20} />,
                  label: "Email Support",
                  val: "care@swadseva.in",
                },
                {
                  icon: <MapPin size={20} />,
                  label: "Location",
                  val: "Sector 5, Kolkata.",
                },
              ].map((item, idx) => (
                <div key={idx} className="flex gap-5 group">
                  ...
                  <div>
                    ...
                    <p className="font-black text-[#1e4a6e] text-lg tracking-tight">
                      {item.val}
                    </p>
                  </div>
                </div>
          ...
          <div className="bg-white border-2 border-slate-100 rounded-[3rem] p-10 md:p-16 shadow-xl shadow-slate-200/20">
            <div className="mb-12">
              ...
              <p className="text-slate-400 text-sm font-medium">
                Our clinical dietician team will review your message.
              </p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="grid md:grid-cols-2 gap-8">
              <div className="space-y-3">
                ...
                <input
                  ...
                  required
                  className="w-full bg-slate-50 border-2 border-slate-50 rounded-2xl p-5 outline-none focus:border-[#75a74c]/30 focus:bg-white transition-all font-medium"
                />
              </div>
              <div className="space-y-3">
                <label className="text-[10px] font-black uppercase text-slate-500 ml-2 tracking-widest">
              ...
              <div className="md:col-span-2 space-y-3">
                ...
                <div className="relative">
                  <select
                    required
                    className="w-full bg-slate-50 border-2 border-slate-50 rounded-2xl p-5 outline-none focus:border-[#75a74c]/30 focus:bg-white transition-all font-bold text-[#1e4a6e] appearance-none"
                  >
              ...
              <button
                type="submit"
                className="md:col-span-2 bg-[#75a74c] text-white py-6 rounded-2xl font-black uppercase text-xs tracking-[0.2em] flex items-center justify-center gap-4 hover:bg-[#1e4a6e] hover:shadow-2xl hover:shadow-[#1e4a6e]/20 transition-all active:scale-95"
              >
